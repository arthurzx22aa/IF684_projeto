...
from uniformCost import uniform_cost_search  # Importando a função
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_LEFT:
                ...
                agent_pos[0] = min(agent_pos[0] + 1, SCREEN_WIDTH // BLOCK_SIZE - 1)
            elif event.key == pygame.K_UP:
                agent_pos[1] = max(agent_pos[1] - 1, 0)
            elif event.key == pygame.K_DOWN:
                agent_pos[1] = min(agent_pos[1] + 1, SCREEN_HEIGHT // BLOCK_SIZE - 1)
            ...
            if event.button == 1:  # left mouse button
                ...
                path, _ = uniform_cost_search(game_map, tuple(agent_pos), food_position)
    ...
    if path:
        if food_position and current_step < len(path):
            if current_time >= next_move_time:
                agent_pos = list(path[current_step])
                current_step += 1
